...
class AfpAusgabe(object):
    ...
    def  __init__(self,  debug = False, data = None, serial_tags = None):
        ...
        self.serial_ignore_count = None # if given, number of lines to beskipped, if no writing happend
        self.serial_line_skipped = None # if not None empty lines will not be written
        if type(data) == list:
            self.serial_index = 0
            self.data = data[0]
            ...
            if serial_tags:
                ...
                if lgh > 3 and serial_tags[3]: self.serial_tag_start_skips = int(serial_tags[3])
                if lgh > 4 and serial_tags[4]: 
                    self.serial_ignore_count =  int(serial_tags[4])
                    self.serial_line_skipped = False
        else:
            self.data = data
    ...
    def loop_serial_text(self):
        ...
        while len(self.serial_data) > self.serial_index+1:
            ...
            if self.serial_line_skipped is None:
                for line in self.serial_text:
                    self.handle_line(line)
            else:
                lgh = len(self.serial_text)
                for i in range(lgh):
                    if self.serial_line_skipped and i < self.serial_ignore_count: continue
                    if i >= self.serial_ignore_count: self.serial_line_skipped = True
                    if self.serial_line_skipped and i > lgh - self.serial_ignore_count - 1: break
                    line = self.serial_text[i]
                    print ("AfpAusgabe.loop_serial_text line:", line) 
                    self.handle_line(line)
                if  self.serial_index > 2: break
    ...
    def write_line(self, lines):
        ...
        if self.serial_line_skipped is None:
            ...
            print ("AfpAusgabe.write_line:", self.serial_line_skipped, fields) 
            if self.serial_line_skipped and fields:
                self.serial_line_skipped = self.no_text_in_line(line)
            print ("AfpAusgabe.write_line execute:", not self.serial_line_skipped) 
            if not self.serial_line_skipped: self.tempfile.write(line)
    ...
    def concat_line(self, fields, netto, quoted = False):
        ...
        return line
    ##check if line holds visible text
    #@param line - line to be checked
    def no_text_in_line(self, line):
        tags, text = Afp_between(line, "<", ">")
        print ("AfpAusgabe.text_in_line:", text)
        for part in text:
            if part.strip(): return False
        return True
