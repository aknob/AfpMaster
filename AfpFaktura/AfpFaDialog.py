...
def AfpLoad_FaArtikelAusw(globals, index, value = "", where = None, ask = False):
    ...
    if ask:
        value, Ok = AfpReq_Text("Bitte Suchbegriff für Artikelauswahl eingeben:".decode("UTF-8"), "", value, "Artikelauswahl")
        if Ok:
    ...
    if Ok:
        ...
        text = "Bitte Artikel auswählen:".decode("UTF-8")
...
class AfpDialog_FaLine(AfpDialog):
    ...
    def InitWx(self):
        ...
        self.button_Abbr = wx.Button(panel, id=wx.ID_CANCEL, label="&Abbruch".decode("UTF-8"), pos=(200,50), size=(70,35), name="BEnde")
