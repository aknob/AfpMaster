...
def AfpACRCalculator_main(conffile, datafile, rangefile, budget_str, ref_str, focal_str, part, check, display, debug):
    ...
    if data and range_lines:
        ...
        if "DATA_INVALID_LIST" in config or "ELIGIBLE_RELATIV_OTE_FACTOR" in config or "ELIGIBLE_OTE_MAXIMUM" in config: 
            ...
            else: ote_factor = None
            if "ELIGIBLE_OTE_MAXIMUM" in config: ote_max= config["ELIGIBLE_OTE_MAXIMUM"]
            else: ote_max = None
            people.set_eligibillity(data_list, ote_factor, ote_max)
...
class AfpACRCalculator(object):
    ...
    def  __init__(self, people, percent, fixum = None, debug = False):
        ...
        self.rangemap = None
        ##  matrix with additional values for promotion
        self.promomap = None
    ...
    def set_manager(self, rule = None, matrix = None, calcpoint = None):
    ...
    ## set promotion matrix
    # @param promo - map or rule for promotion Zuschlag
    def set_promomap(self, promo):
        n = self.people.get_matrix_length(0)
        m = self.people.get_matrix_length(1)
        if Afp_isNumeric(promo):
            self.promo = promo
    ...
    def gen_promotion_sum(self, fte = True):
        promo = 0.0
        if self.promomap:
            matrix = self.people.get_promotion_matrix(fte)
            n = self.people.get_matrix_length(0)
            m = self.people.get_matrix_length(1)
            for i in range(m):
                for j in range(n):
    ...
    def generate_reference_matrix(self):
        ...
        if matrix:
            ...
            if self.debug: print "AfpACRCalculator.generate_reference_matrix:", self.reference_value, matrix[m/2][n/2], "\n old:",self.reference_matrix,"\n new:", matrix
            self.reference_matrix = matrix
            self.reference_value = matrix[m/2][n/2]    
    ...
    def generate_acr_matrix(self):
        ...
        if matrix:
            ...
            self.matrix = matrix
    ## generate promotion matrix \n
    def generate_promo_matrix(self):
        matrix = None
        ...
        if from_matrix:
            ...
            for i in range(len(matrix)):
                for j in range(len(matrix[i])):
                    if Afp_validMatrixField(i, j, self.promomap):
                        matrix[i][j] =  from_matrix[i][j] + self.promomap[i][j]
                    else:
                        matrix[i][j] =  from_matrix[i][j] + self.promo
        if matrix:
    ...
    def check_increase(self):
        if self.check_matrix:
            ...
            promo_matrix = self.people.get_promotion_matrix(True, True)
            intend_sum = 0.0
            check_sum = 0
            print
            print "AfpACRCalculator.check_increase promotion values:", promo_matrix
            ...
            for i in range(len(matrix)):
                for j in range(len(matrix[i])):
                    if Afp_validMatrixField(i, j, self.check_matrix):
                        check = self.check_matrix[i][j]*self.focal_value/100 +  promo_matrix[i][j]
                    else:
                        check = 0.0
                    intend = self.ote_matrix[i][j] *self.focal_matrix[i][j]/100 + promo_matrix[i][j]
                    promo_sum += promo_matrix[i][j]
                    check_sum += check
                    intend_sum += intend
            ...
            print "%s %f \t %f\t%f\t%f \t%f \tBugdet: %f" % ("AfpACRCalculator.check_increase sums:", check_sum, intend_sum, matrix_sum, diff_sum, promo_sum, self.budget)
    ...
    ## retrieve  promomap
    def get_promomap(self):
        if self.promo and not self.promomap:
            n = self.people.get_matrix_length(0)
            m = self.people.get_matrix_length(1)
            return Afp_initMatrix(n, m, self.promo)
        else:
            return self.promomap
    ...
    def get_matrix(self, ident):
        if "ACR" in ident:
            ...
            if ident == "PROMO - Focalvalues":
                return self.promo_matrix
            elif ident == "PROMO":
                return self.get_promomap()
        elif "OTE" in ident:
    ...
    def view(self, typ = None):
        ...
        if typ is None or typ == "MATRIX":
            ...
            print "AfpACRCalculator.view promotion matrix:", self.promo_matrix
            OTE = self.people.get_ote_sum()
        ...
        for line in range_lines:
            ...
            if current_range:
                ...
                if cols[colmap["NAME"]]: 
                    next_range = cols[colmap["NAME"]].strip()
        self.ranges = ranges
        self.midindex = (colmap["MAX"] - colmap["MIN"])/2
        if debug: self.view()
    ...
    def get_range_target(self, family, clg, dait):
        if family in self.ranges:
            ...
            if clg in ranges:
                rang = ranges[clg]
                if dait == "I" or dait =="T":
                    return rang[self.midindex], rang[self.midindex+1]
                else:
                    return rang[self.midindex-1], rang[self.midindex]
            else:
        ...
        if not intervals is None: 
            ...
            for val in intervals:
                if value and value < val:
                    ...
                    value = None
            ind -= lgh/2
        #print "AfpACRRanges.get_relativ_to_target:", value, intervals, ind
        return ind, intervals
    ## show data
    def view(self):
...
class AfpACRPerson(object):
    ...
    # @param promo - flag if additional promotion increase should be returned
    def get_increase(self, fte = True, promo = False):
        if promo:  inc = self.promo_increase
        elif fte:  inc = self.OTE_increase
        else:  inc = self.increase
        if inc is None: inc = 0.0
        #if promo: print "AfpACRPerson.get_increase:", promo, inc, self.promo_increase
        return inc
    ...
    def get_row(self):
        ...
        return [Afp_fromString(self.name), self.family, Afp_toString(self.OTE), Afp_toString(self.fte), Afp_toString(self.clg), self.dait, Afp_toPos(self.positioning), Afp_toString(self.performance), promo, percent, increase]
    ## show data
...
class AfpACRPeopleManager(object):
    ...
    def ote_above_rangemid(self, fac=None, list=False):
        ...
        for person in self.persons:
            if person.is_eligible():
                ...
                elif person.get_performance() == -1:
                    below += 1
                count +=1
        percab= 100.0 *cnt/count
   ...
   ## set positioning due to input ranges
    ...
    def set_eligibillity(self, invalidlist, max_ote_factor = None, max_ote = None):
        ...
        for person in self.persons:
            person.set_eligibillity(invalidlist, max_ote_factor, max_ote)
    ...
    def get_matrix_length(self, dir):
        return self.matrix_range[dir][1] - self.matrix_range[dir][0] + 1
    ...
    def get_matrix_index(self, dir, pos):
        ...
        if dir == 0: pos *= -1
        return pos - self.matrix_range[dir][0] 
   ...
   ## generate increase sum
    ...
    def get_ote_sum(self, fte = True, new = False, all = True):
        ...
        return sum
    ## generate promotion ote sum matrix
    # @param fte - flag if fulltime ote should be used
    # @param inc - flag if additional increase should be used instead of ote
    def get_promotion_matrix(self, fte = True, inc = False):
        sum = 0.0
        ...
        for person in self.persons:
            if person.get_promotion() and person.is_eligible():
                ...
                j = self.get_matrix_index(0, person.get_positioning())
                if inc:
                    matrix[i][j] += person.get_increase(fte, True)
                else:
