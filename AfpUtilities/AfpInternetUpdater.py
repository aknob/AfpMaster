...
def Afp_InternetUpdater(config, debug):
    ...
    if Afp_existsFile(config):
        ...
        ok = True
        while not ok == False:
            results = AfpReq_MultiLine( "Bitte Updatekonfiguration für das Internet auswählen.".decode("UTF-8"), "", "Button", liste, "Internet Update", 300)
            if results:
                result = ""
                for res in results:
                    if res: result = res
...
def Afp_fromHtml(text):
    text = text.replace("&auml;", "ä".decode("UTF-8"))
    text = text.replace("&ouml;", "ö".decode("UTF-8"))
    text = text.replace("&uuml;", "ü".decode("UTF-8"))
    text = text.replace("&Auml;", "Ä".decode("UTF-8"))
    text = text.replace("&Ouml;", "Ö".decode("UTF-8"))
    text = text.replace("&Uuml;", "Ü".decode("UTF-8"))
    text = text.replace("&szlig;", "ß".decode("UTF-8"))
    return text
## convert german 'Umlaute'  in UTF-8 to html-representation
# @param text - text to be converted
def Afp_toHtml(text):
    text = text.replace("ä".decode("UTF-8"), "&auml;")
    text = text.replace( "ö".decode("UTF-8"), "&ouml;")
    text = text.replace( "ü".decode("UTF-8"), "&uuml;")
    text = text.replace( "Ä".decode("UTF-8"), "&Auml;")
    text = text.replace( "Ö".decode("UTF-8"), "&Ouml;")
    text = text.replace( "Ü".decode("UTF-8"), "&Uuml;")
    text = text.replace( "ß".decode("UTF-8"), "&szlig;")
    return text
...
class AfpFTPConnector(object):
    ...
    def invoke_ftp(self, user=None, word=None):
        ...
        if word is None and "ftp-word" in self.settings:
            word = self.settings["ftp-word"].decode("base64")
        if self.FTP and  user and word:
            r1 = self.FTP.connect(self.server, self.server_port)
            r2 = self.FTP.login(user,word.decode("base64"))
            self.connected = True
            if self.remotedir:
...
class AfpDialog_calEvents(AfpDialog):
    ...
    def __init__(self, *args, **kw):   
        ...
        self.SetTitle("Termineinträge".decode("UTF-8"))
        self.Bind(wx.EVT_SIZE, self.On_ReSize)
        #self.Bind(wx.EVT_ACTIVATE, self.On_Activate)
        
    ## set up dialog widgets      
    def InitWx(self):
        self.label_Label = wx.StaticText(self, 1, label="Verwaltung der Termineinträge für".decode("UTF-8"), name="LLabel")
        ...
        self.grid_events.SetSelectionMode(wx.grid.Grid.wxGridSelectRows)   
    ...
    def attach_data(self, data, new = False, editable = False):
        ...
        self.SetTitle(data.get_type().decode("UTF-8"))
   ...
   ## Event handler to delete events
    def On_delete(self, event):
        ...
        if indices:
            ...
            ok = AfpReq_Question("Keine Termine ausgewählt,".decode("UTF-8"), "soll die gesamte Liste gelöscht werden?".decode("UTF-8"), "Löschung".decode("UTF-8"))
            if ok:
                self.data.clear()
                self.Pop_Events()   
        event.Skip()
   ...
   ## Load data from selected file
    def On_Load(self, event):
        ...
        filename, Ok = AfpReq_FileName(dir,"Termindatei öffnen".decode("UTF-8"), "*.js", True)
        if Ok:
   ...
   ## Save dialog entries to local file
    def On_Save(self, event):
        ...
        dir = self.data.localpath
        filename, Ok = AfpReq_FileName(dir,"Termindatei speichern".decode("UTF-8"), "*.js")
        if Ok:
...
class AfpDialog_Sildeshow(AfpDialog):
    ...
    ## set up dialog widgets      
    def InitWx(self):
        self.label_Label = wx.StaticText(self, 1, label="Erzeugung einer Slideshow für".decode("UTF-8"), name="LLabel")
        self.label_Name = wx.StaticText(self, 1, label="", name="Name")
        self.top_line1_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.top_line1_sizer.Add(self.label_Label,0,wx.EXPAND)
        self.top_line1_sizer.AddSpacer(5)
    ...
    def write_data(self):
        ...
        if ok and self.data.show_exists():
            ok = AfpReq_Question("Die Slideshow '" + slidename + "' existiert schon!", "Soll diese Slideshow überschrieben werden?".decode("UTF-8"), "Speichern")
            if not ok:
                pathname, ok = AfpReq_FileName(self.data.get_localpath(False),"Bitte Verzeichnis auswählen, in das gespeichert werden soll.".decode("UTF-8"), None)
                if ok and pathname:
                    self.data.set_localpath(pathname)
        elif not ok:
            AfpReq_Info("Die Slideshow '" + slidename + "' ist nicht geändert worden,".decode("UTF-8"), "Sie wird nicht gespeichert!")
        if ok:
            self.data.save_slideshow()
            #ok = AfpReq_Question("Die Slideshow '" + slidename + "' wurde gespeichert!", "Soll diese Slideshow in die Präsentation eingehängt werden?".decode("UTF-8"), "Speichern")
            #if ok:
    ...
    def On_Change(self, event):
        ...
        if show == "- Neu -":
            ...
            show = ""
            show, Ok = AfpReq_Text("Bitte die Bezeichnung (Name oder Ort) für die neue Slideshow eingeben.".decode("UTF-8"), "Wenn die beiden letzten Zeichen nicht die aktuelle Jahreszahl darstellen,\n wird das aktuelle Jahr angehängt (_20xx).".decode("UTF-8"), show)
            if show and Ok:
                show = show.replace(" ","_")
   ...
   ## Event handler for double click Show Listcontrol
    def On_DClick_Show(self, event=None):
        ...
        text, Ok = AfpReq_Text("", "Bitte Text für das ausgewählte Bild eingeben:".decode("UTF-8"), text)
        if Ok:
    ...
    def On_Add(self, event=None):
        ...
        if max:
            ...
            if self.data.get_image_len() >= max: 
                AfpReq_Info("Maximale Anzahl von Bildern in der Slideschow erreicht,", "bitte ein Bild löschen bevor ein weiteres hinzugefügt werden kann.".decode("UTF-8"))
                row = -1
        if row >= 0:
            sel = self.list_Show.GetFirstSelected()
            #print "AfpDialog_Sildeshow.On_add:", row, sel
            if sel >= 0:
                label = "Bild vor der ausgewählten Position(".decode("UTF-8") + Afp_toString(sel+1) + ") in die Slideshow anfügen?".decode("UTF-8")
            else:
                label = "Bild am Ende der Slideshow anfügen?".decode("UTF-8")
            text = ""
            text, Ok = AfpReq_Text(label, "Bitte Text für das ausgewählte Bild eingeben:".decode("UTF-8"), text)
            if Ok:
    ...
    def On_Delete(self, event=None):
        ...
        sel = self.list_Show.GetFirstSelected()
        if sel >= 0:
            ok = AfpReq_Question("Ausgewähltes Bild".decode("UTF-8"), "aus der Slideshow löschen?".decode("UTF-8"), "Löschung".decode("UTF-8"))
            if ok:
    ...
    def On_Get(self, event):
        ...
        results = AfpReq_MultiLine( "Bitte auswählen was vom Server heruntergeladen werden soll!".decode("UTF-8"), "Dafür bitte den ensprechenden Knopf drücken.".decode("UTF-8"), "Button", liste, "Vom Server holen", 300)
        if results:
            result = ""
            for res in results:
                if res: result = res
   ...
   ## Load data from selected file
    def On_Load(self, event):
        ...
        results = AfpReq_MultiLine( "Auswählen was neu geladen werden soll!".decode("UTF-8"), "Bitte den ensprechenden Knopf drücken.".decode("UTF-8"), "Button", liste, "Laden", 300)
        if results:
            result = ""
            for res in results:
                if res: result = res
            if result == "Slideshow":
                ...
                pathname, Ok = AfpReq_FileName(dir,"Bitte Slideshow auswählen, die geladen werden soll.".decode("UTF-8"), None)
                if pathname[-5:] == ".html":
                    pathname = pathname[:-5] 
                ...
                pathname, Ok = AfpReq_FileName(dir,"Bitte Verzeichnis auswählen, das geladen werden soll.".decode("UTF-8"), None)
                #print "AfpDialog_Sildeshow.On_Load:", pathname, Ok
                if Ok and pathname:
   ...
   ## Save dialog entries to local file and send file to remote site
    def On_Send(self, event):
        ...
        if ok:
            slidename = self.data.get_localname()
            ok = AfpReq_Question("Die Slideshow '" + slidename + "' lokal gesichert,", "soll diese Slideshow auf den Internet-Server übetragen werden?".decode("UTF-8"), "FTP-Upload")
